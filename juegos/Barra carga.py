...
# Solicitar al usuario el tamaño del archivo en MB
tamaño_archivo = float(input("Ingrese el tamaño del archivo (MB): "))

# Solicitar al usuario el tiempo total de carga en segundos
tiempo_total = float(input("Ingrese el tiempo de carga (segundos): "))

# Mostrar mensaje de inicio
print(f"\nIniciando subida de {tamaño_archivo} MB...\n")

# Definir cuántos incrementos de progreso habrá (de 5% en 5%)
incrementos = 20
# Calcular el tiempo que debe pasar entre cada actualización
tiempo_por_incremento = tiempo_total / incrementos

# Bucle que itera 20 veces (0% a 100% en intervalos de 5%)
for i in range(1, incrementos + 1):
    # Calcular el porcentaje actual
    porcentaje = (i * 100) // incrementos
    
    # Calcular cuántos caracteres "#" mostrar (20 caracteres totales)
    caracteres_llenos = (i * 20) // incrementos
    # Calcular cuántos caracteres "-" mostrar (espacios vacíos)
    caracteres_vacios = 20 - caracteres_llenos
    
    # Construir la barra de progreso
    barra = "[" + "#" * caracteres_llenos + "-" * caracteres_vacios + "]"
    
    # Mostrar la barra con el porcentaje (usando \r para actualizar la misma línea)
    print(f"\r{barra} {porcentaje}%", end="", flush=True)
    
    # Pausar el tiempo correspondiente antes de la siguiente actualización
    time.sleep(tiempo_por_incremento)

# Saltar a una nueva línea después de completar
print()
