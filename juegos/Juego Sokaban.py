...
def mover(mapa, direccion):
    ...
    nueva_fila = fila + mover_fila
    nueva_columna = columna + mover_columna
    ...
    # ======================
        ...
        caja_fila = nueva_fila + mover_fila
        caja_columna = nueva_columna + mover_columna

        # Validar límites
        if caja_fila < 0 or caja_columna < 0:
            return

        if caja_fila >= len(mapa):
            return

        if caja_columna >= len(mapa[0]):
            return

        siguiente_caja = mapa[caja_fila][caja_columna]
